...
import { commentsUpvotesTable, postUpvotesTable } from "@/db/schemas/upvotes";
  ...
  .get(
    ...
    zValidator(
      ...
      paginationSchema.extend({
        includeChildren: z.coerce.boolean().default(true).optional(),
      }),
    ),
    async (c) => {
      const user = c.get("user")!;
      const { id } = c.req.valid("param");
      const { limit, page, order, sortBy, includeChildren } =
        c.req.valid("query");
      const offset = (page - 1) * limit;
